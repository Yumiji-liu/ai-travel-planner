...
import { 
  Share2, Edit3, ChevronLeft, Calendar, MapPin, 
  Users, DollarSign, Tag, ExternalLink, Copy, 
  Check, Filter
} from 'lucide-react';
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <Link 
                href="/dashboard" 
                className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-teal-600 mb-3 transition-colors"
              >
              ...
              {trip.travelStyles.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {trip.travelStyles.map(style => (
                    <span key={style} className="inline-flex items-center gap-1 rounded-full bg-teal-50 px-3 py-1 text-xs font-medium text-teal-700 capitalize">
                      {style}
                    </span>
                  ))}
                </div>
...
export default function TripPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  return <TripPageContent tripId={id} />;
}
